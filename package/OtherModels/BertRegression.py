...
from package.OtherModels.bertregression_inference_script import inference_bert

def train():
    st.write('https://huggingface.co/course/chapter3/3?fw=pt')
    st.markdown('#### Training Script')
    with open('./package/OtherModels/bertregression_train_script.py', 'r') as fp:
        data = fp.read()
    ...
    if train_button: 
        with st.spinner('Training the model...'):          
            matrix = subprocess.run([f"{sys.executable}", "./package/OtherModels/bertregression_train_script.py"],
...
def inference():
    ...
    with open('./package/OtherModels/bertregression_inference_script.py', 'r') as fp:
        data = fp.read()
         
    st.code(data, language='python')

    text1, text2, button_clicked = try_now('Bert as a Regression')
    if button_clicked:
